...
#if TARGET_OS_IPHONE
#import "WDMPMediaLibraryAssetLoader.h"
#endif /* TARGET_OS_IPHONE */
...
@interface _WDAudioLocalFileProvider : WDAudioFileProvider
@end

@interface _WDAudioRemoteFileProvider : WDAudioFileProvider {
@private
  WDSimpleHTTPRequest *_request;
  NSURL *_audioFileURL;
...
@private
  WDMPMediaLibraryAssetLoader *_assetLoader;
...
@implementation _WDAudioLocalFileProvider
...
@implementation _WDAudioRemoteFileProvider
...
- (void)_createRequest
{
  _request = [WDSimpleHTTPRequest requestWithURL:_audioFileURL];
  if (_audioFileHost != nil) {
    [_request setHost:_audioFileHost];
  }
  __unsafe_unretained _WDAudioRemoteFileProvider *_self = self;
...
static void audio_file_stream_property_listener_proc(void *inClientData,
                                                ...
                                                UInt32 *ioFlags)
{
  __unsafe_unretained _WDAudioRemoteFileProvider *fileProvider = (__bridge _WDAudioRemoteFileProvider *)inClientData;
...
{
  _assetLoader = [WDMPMediaLibraryAssetLoader loaderWithURL:[_audioFile audioFileURL]];
...
+ (instancetype)_fileProviderWithAudioFile:(id <WDAudioFile>)audioFile
{
  ...
  if ([audioFileURL isFileURL]) {
    return [[_WDAudioLocalFileProvider alloc] _initWithAudioFile:audioFile];
  }
#if TARGET_OS_IPHONE
    ...
    return [[_WDAudioRemoteFileProvider alloc] _initWithAudioFile:audioFile];
  }
}
