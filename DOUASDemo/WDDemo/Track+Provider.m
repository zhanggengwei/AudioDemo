...
+ (NSArray *)remoteTracks
{
  ...
  dispatch_once(&onceToken, ^{
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:@"https://douban.fm/j/mine/playlist?type=n&channel=1004693&from=mainsite"]];
    NSData *data = [NSURLConnection sendSynchronousRequest:request
                                         returningResponse:NULL
                                                     error:NULL];
    NSString *string = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
      
     NSString * path = [[NSBundle mainBundle]pathForResource:@"song" ofType:@"txt"];
      NSDictionary * dict = [NSDictionary dictionaryWithContentsOfFile:path];
      NSError * error;
      NSData * content =[NSData dataWithContentsOfFile:path];
    NSMutableArray *allTracks = [NSMutableArray array];
      dict =[NSJSONSerialization JSONObjectWithData:content options:NSJSONReadingMutableLeaves error:&error];
...
+ (NSArray *)musicLibraryTracks
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    for (MPMediaItem *item in [[MPMediaQuery songsQuery] items]) {
      ...
      track.audioFileURL = [NSURL fileURLWithPath:[[NSBundle mainBundle]pathForResource:@"abc" ofType:@"aac"]];
