...
@private
  ...
  WDAudioStreamer *_streamer;
  WDAudioVisualizer *_audioVisualizer;
}
@end
...
- (void)loadView
{
  ...
  _audioVisualizer = [[WDAudioVisualizer alloc] initWithFrame:CGRectMake(0.0, CGRectGetMaxY([_volumeSlider frame]), CGRectGetWidth([view bounds]), CGRectGetHeight([view bounds]) - CGRectGetMaxY([_volumeSlider frame]))];
...
- (void)_resetStreamer
{
  [self _cancelStreamer];
    ...
    else
    {
        ...
        _streamer = [WDAudioStreamer streamerWithAudioFile:track];
...
- (void)_setupHintForStreamer
{
  ...
  [WDAudioStreamer setHintWithAudioFile:[_tracks objectAtIndex:nextIndex]];
}
...
- (void)viewWillAppear:(BOOL)animated
{
  ...
  [_volumeSlider setValue:[WDAudioStreamer volume]];
}
...
{
  if ([_streamer status] == WDAudioStreamerPaused ||
      [_streamer status] == WDAudioStreamerIdle) {
    [_streamer play];
  }
  else {
    [_streamer pause];
...
{
  [WDAudioStreamer setVolume:[_volumeSlider value]];
}
