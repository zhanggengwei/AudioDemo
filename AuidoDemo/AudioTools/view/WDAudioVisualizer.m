...
@interface WDAudioVisualizer () {
@private
  struct {
    float current[kWDAudioAnalyzerLevelCount];
    float last[kWDAudioAnalyzerLevelCount];
    float pacing[kWDAudioAnalyzerLevelCount];
  } _levels;

  float _coefficient;
  NSUInteger _step;
  NSUInteger _stepCount;
  WDAudioVisualizerInterpolationType _interpolationType;
...
@implementation WDAudioVisualizer
...
+ (WDAudioAnalyzer *)_sharedAnalyzer
{
  static WDAudioAnalyzer *sharedAnalyzer = nil;

  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    sharedAnalyzer = [WDAudioAnalyzer frequencyAnalyzer];
    [sharedAnalyzer setEnabled:YES];
    [WDAudioStreamer setAnalyzers:@[sharedAnalyzer]];
...
- (void)_setupVisualizer
{
  ...
  _step = 0;
  _stepCount = 6;
  _interpolationType = WDAudioVisualizerLinearInterpolation;
}
...
#define INTERPOLATED_COEFFICIENT_SMOOTH (sinf(COEFFICIENT * M_PI - M_PI_2) * 0.5f + 0.5f)
  ...
  default:
  case WDAudioVisualizerLinearInterpolation:
      _coefficient = INTERPOLATED_COEFFICIENT_LINEAR;
    break;

  case WDAudioVisualizerSmoothInterpolation:
...
#undef INTERPOLATED_COEFFICIENT_SMOOTH
  ...
  if (++_step > _stepCount) {
    _step = 0;
    memcpy(_levels.last, _levels.current, sizeof(float) * kWDAudioAnalyzerLevelCount);
