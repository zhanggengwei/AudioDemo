...
#import "WDAudioPlaybackItem.h"
#import "WDAudioLPCM.h"
#import "WDAudioDecoder.h"
#import "WDAudioRenderer.h"
#include <sys/types.h>
#include <sys/event.h>
#include <sys/time.h>
#include <pthread.h>
...
@interface WDAudioEventLoop () {
@private
    WDAudioRenderer *_renderer;
    WDAudioStreamer *_currentStreamer;
    
    NSUInteger _decoderBufferSize;
    WDAudioFileProviderEventBlock _fileProviderEventBlock;
...
@implementation WDAudioEventLoop
...
{
    static WDAudioEventLoop *sharedEventLoop = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedEventLoop = [[WDAudioEventLoop alloc] init];
    });
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _renderer = [WDAudioRenderer rendererWithBufferTime:kWDAudioStreamerBufferTime];
        [_renderer setUp];
        
        if ([[NSUserDefaults standardUserDefaults] objectForKey:kWDAudioStreamerVolumeKey] != nil) {
            [self setVolume:[[NSUserDefaults standardUserDefaults] doubleForKey:kWDAudioStreamerVolumeKey]];
        }
        else {
            [self setVolume:1.0];
        }
...
+ (NSUInteger)_decoderBufferSize
{
    AudioStreamBasicDescription format = [WDAudioDecoder defaultOutputFormat];
    return kWDAudioStreamerBufferTime * format.mSampleRate * format.mChannelsPerFrame * format.mBitsPerChannel / 8 / 1000;
}
...
{
    __unsafe_unretained WDAudioEventLoop *eventLoop = (__bridge WDAudioEventLoop *)inClientData;
...
{
    __unsafe_unretained WDAudioEventLoop *eventLoop = self;
...
- (BOOL)_handleEvent:(event_type)event withStreamer:(WDAudioStreamer **)streamer
{
    if (event == event_play) {
        if (*streamer != nil &&
            ([*streamer status] == WDAudioStreamerPaused ||
             [*streamer status] == WDAudioStreamerIdle ||
             [*streamer status] == WDAudioStreamerFinished)) {
...
# pragma clang diagnostic pop
                    if (status == noErr) {
#endif /* TARGET_OS_IPHONE */
                        [*streamer setStatus:WDAudioStreamerPlaying];
        ...
        if (*streamer != nil &&
            ([*streamer status] != WDAudioStreamerPaused &&
             ...
             [*streamer status] != WDAudioStreamerFinished)) {
                [_renderer stop];
                [*streamer setStatus:WDAudioStreamerPaused];
            }
    }
    else if (event == event_stop) {
        if (*streamer != nil &&
            [*streamer status] != WDAudioStreamerIdle) {
            if ([*streamer status] != WDAudioStreamerPaused) {
                [_renderer stop];
            }
            [_renderer flush];
            [*streamer setDecoder:nil];
            [*streamer setPlaybackItem:nil];
            [*streamer setStatus:WDAudioStreamerIdle];
        }
    }
        ...
        *streamer = _currentStreamer;
        ...
        if (*streamer != nil &&
            [*streamer status] == WDAudioStreamerBuffering) {
            [*streamer setStatus:WDAudioStreamerPlaying];
        }
...
- (void)_handleStreamer:(WDAudioStreamer *)streamer
{
    if (streamer == nil) {
        return;
    }
    
    if ([streamer status] != WDAudioStreamerPlaying) {
        return;
    }
    
    if ([[streamer fileProvider] isFailed]) {
        [streamer setError:[NSError errorWithDomain:kWDAudioStreamerErrorDomain
                                               code:WDAudioStreamerNetworkError
                                           userInfo:nil]];
        [streamer setStatus:WDAudioStreamerError];
        return;
    }
    
    if (![[streamer fileProvider] isReady]) {
        [streamer setStatus:WDAudioStreamerBuffering];
        return;
    }
    
    if ([streamer playbackItem] == nil) {
        [streamer setPlaybackItem:[WDAudioPlaybackItem playbackItemWithFileProvider:[streamer fileProvider]]];
        if (![[streamer playbackItem] open]) {
            [streamer setError:[NSError errorWithDomain:kWDAudioStreamerErrorDomain
                                                   code:WDAudioStreamerDecodingError
                                               userInfo:nil]];
            [streamer setStatus:WDAudioStreamerError];
            return;
        }
    ...
    if ([streamer decoder] == nil) {
        [streamer setDecoder:[WDAudioDecoder decoderWithPlaybackItem:[streamer playbackItem]
    ...
    switch ([[streamer decoder] decodeOnce]) {
        case WDAudioDecoderSucceeded:
            break;
            
        case WDAudioDecoderFailed:
            [streamer setError:[NSError errorWithDomain:kWDAudioStreamerErrorDomain
                                                   code:WDAudioStreamerDecodingError
                                               userInfo:nil]];
            [streamer setStatus:WDAudioStreamerError];
        ...
        case WDAudioDecoderEndEncountered:
            [_renderer stop];
            [streamer setDecoder:nil];
            [streamer setPlaybackItem:nil];
            [streamer setStatus:WDAudioStreamerFinished];
            return;
            
        case WDAudioDecoderWaiting:
            [streamer setStatus:WDAudioStreamerBuffering];
            return;
    }
...
- (void)_eventLoop
{
    WDAudioStreamer *streamer = nil;
    
    while (1) {
        @autoreleasepool {
            if (streamer != nil) {
                switch ([streamer status]) {
                    case WDAudioStreamerPaused:
                    case WDAudioStreamerIdle:
                    case WDAudioStreamerFinished:
                    case WDAudioStreamerBuffering:
                    case WDAudioStreamerError:
...
static void *event_loop_main(void *info)
{
    ...
    __unsafe_unretained WDAudioEventLoop *eventLoop = (__bridge WDAudioEventLoop *)info;
    @autoreleasepool {
...
- (void)setCurrentStreamer:(WDAudioStreamer *)currentStreamer
{
...
- (void)setVolume:(double)volume
{
    ...
    if ([WDAudioStreamer options] & WDAudioStreamerKeepPersistentVolume) {
        [[NSUserDefaults standardUserDefaults] setDouble:volume
                                                  forKey:kWDAudioStreamerVolumeKey];
    }
}
